...
if (!$tablecheck = mysqli_query($_SERVER['con'],"SELECT * FROM `election_users`"))
{
	//create user table
	$createusertable = "
	...
	if (!$addidex = mysqli_query($_SERVER['con'], "ALTER TABLE `election_users` ADD PRIMARY KEY (`id`)")) redalert("ERROR: Could not create primary key on users table");
	if (!$addidex = mysqli_query($_SERVER['con'], "ALTER TABLE `election_users` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;")) redalert("ERROR: Could not create primary key on users table");
	if (!$addidex = mysqli_query($_SERVER['con'], "ALTER TABLE `election_users` ADD UNIQUE(`firstname`, `lastname`, `email`);")) redalert("ERROR: Could not create unique key on users table");
	if (!$addidex = mysqli_query($_SERVER['con'], "ALTER TABLE `election_users` ADD UNIQUE(`email`);")) redalert("ERROR: Could not create unique email key on users table");
}
...
if ($_POST['submit'] == 'Create Admin') if (!$admincheck = mysqli_fetch_array(mysqli_query($_SERVER['con'],"SELECT * FROM `election_users` WHERE `is_admin` = 'Y'")))
...
if ($_POST['submit'] == 'Create Admin') if (!$admincheck = mysqli_fetch_array(mysqli_query($_SERVER['con'],"SELECT * FROM `election_users` WHERE `is_admin` = 'Y'")))
{
	...
	//generate encrypted password using email, password and random string
